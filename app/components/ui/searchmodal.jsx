...
const searchmodal = () => {
    ...
    return (
        <div className='absolute top-0 h-full w-full '>
            <div className='capitalize h-full w-full' onClick={() => setOpen(true)}></div>
                ...
                <ModalDialog className=' p-0 m-0 rounded-xl overflow-hidden w-[600px] '>
                    ...
                    <div className='h-[500px] overflow-y-auto overflow-x-hidden'>
                        {cards.map((item, index3) => {
                            ...
                            return (item.name) && (Text.length == 0 || (item.name).includes(Text)) && <div key={index3} className=' h-[50px] w-[600px] p-2 mb-2 flex items-center hover:bg-slate-100 cursor-pointer'
                                onClick={carduserhandle}
                            >
                                <div className='relative size-10  rounded-full mx-2 overflow-hidden'>
                                    <img className='w-full h-full object-cover' src={item.userpic} alt="" />
                                </div>
                                <div className=''>
                                    <div>{item.name}</div>
